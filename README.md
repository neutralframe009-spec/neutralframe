...
├── photograph/                        # 個別記事フォルダ
│   ├── README.md
│   ├── templateA.html                 # 雛形: Single（見出し→写真縦積み）
│   ├── templateB.html                 # 雛形: Split（写真左・テキスト右）
│   ├── 003.html                     # 記事（templateA）
│   ├── 002.html                     # 記事（templateB）
│   └── 001.html                     # 記事（templateA）
├── about.html                         # ABOUT（プロフィール・機材）
├── lab.html                           # LAB（AI協調制作・自動化などの実験記録）
├── essay.html                         # ESSAY（外部 note.com への導線）
├── assets/
│   ├── styles.css                     # CSS 正本（編集はここ）
│   ├── styles.vN.css                  # 公開用（HTML が参照・自動生成）
│   ├── top-slideshow.js               # JS 正本
│   ├── top-slideshow.vN.js            # 公開用（自動生成）
│   ├── photograph-page.js             # JS 正本
│   ├── photograph-page.vN.js          # 公開用（自動生成）
│   └── version.txt                    # 現在の N
├── scripts/
│   └── bump-asset-version.sh          # CSS/JS 更新後に実行 → vN+1 へ
├── images/
│   ├── README.md
│   └── photograph/                    # ★サイト全体で共有する唯一の画像プール
│       ├── README.md                  # 命名規則・運用
│       ├── 003_01_tokyo_2026_0417.jpg
│       ├── 002_01_tokyo_2020_0128.JPG
│       ├── 001_01_TOKYO_2019_0618.JPG
│       └── ...
└── README.md                          # このファイル
```

写真マニフェスト用のビルドスクリプトは**持たない**。
CSS/JS のキャッシュ対策だけ `scripts/bump-asset-version.sh` を使う（後述）。
...
- 記事を追加するときは `<a class="idx-row" data-photo="NNN" href="photograph/NNN.html">…</a>` を1つ増やすだけ。`photograph/NNN.html` を追加・保存した瞬間に、一覧も TOP も追従する（ビルド／コマンド操作は不要）

### 5.4 `photograph/NNN.html` — 個別記事

`photograph/` フォルダに格納。2種類の雛形を使い分け：

#### templateA — Single（`photograph/templateA.html`）
- 和文タイトル（主・大）→ 英字サブ（小・斜体）→ 4:3 写真 → 2カラム（左：メタ情報 / 右：本文テキストエリア）
- 本文は複数段落可。`<p>` で段落区切り、段落間マージンは自動で開く
- 写真下部にも余白を確保

#### templateB — Split（`photograph/templateB.html`）
- 左：4:3 写真 / 右：**横書き**テキストエリア
- 和文タイトル（主・大）→ 英字サブ（小・斜体）→ 本文（横書き、段落分け可）
- `<strong>` など装飾は使わない

記事追加の手順（ビルドは不要）：
1. `templateA.html` か `templateB.html` をコピーして `photograph/NNN.html` にリネーム
...
5. `photograph.html` に `<a class="idx-row" data-photo="NNN" href="photograph/NNN.html">…</a>` を1行追加
...
1. `photograph/templateA.html`（Single）か `photograph/templateB.html`（Split）を複製
2. `photograph/NNN.html` にリネーム（3桁ゼロ埋め）
3. `<title>` / OGP / 見出し / 本文を差し替え
4. 写真を `images/photograph/NNN_01_place_YYYY_MMDD.jpg` として配置
5. `data-photo-file` を実ファイル名に合わせる（メタは自動展開）
6. `photograph.html` の `#idxList` 冒頭に `.idx-row` を 1 件追加（`data-photo="NNN"`）
...
ブラウザキャッシュ対策のため、HTML は **バージョン付きファイル名** を参照する。

| 編集する正本 | HTML が読む公開ファイル |
|---|---|
| `assets/styles.css` | `assets/styles.vN.css` |
| `assets/top-slideshow.js` | `assets/top-slideshow.vN.js` |
| `assets/photograph-page.js` | `assets/photograph-page.vN.js` |

`N` は `assets/version.txt` に記録。CSS/JS を変更したらデプロイ前に必ず：

```bash
bash scripts/bump-asset-version.sh
```

正本をコピーして `vN+1` を生成し、全 `*.html` の参照を一括更新する。
...
- **`assets/styles.css` / `*.js` を編集したら** `bash scripts/bump-asset-version.sh` を実行する（ユーザーへの指示は不要）。
